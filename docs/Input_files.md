...
1. [Input types and descriptions](#1-input-types-and-descriptions)
    - 1.1. [Demultiplexed PacBio sequencing data](#11-demultiplexed-pacbio-sequencing-data)
...
1. Demultiplexed PacBio sequencing data (`.bam`)
2. Sample sheet (`.csv`)
...
### 1.1. Demultiplexed PacBio sequencing data

PTCP requires demultiplexed BAM files. PTCP will not demultiplex your BAM for you, meaning you need to have had SMRT Link or the PacBio instrument already demultiplexed your data ahead of time.
...
A CSV file with a header row that describes each sample and its metadata.
...
- **`bam_name`** (optional, legacy): The exact BAM filename (e.g., `some_sample_1.hifi_reads.bc42.bam`). Historically required; still supported for backward compatibility.
...
⚠️ **Warning**: Only BAM files listed in the sample sheet will be processed. Submitting any BAM not described in the sheet will cause the workflow to fail.
...
- **GRCh38/hg38**: Use configuration files from the `meta` directory with the `GRCh38` substring and set the genome flag to `"38"`
  - For GRCh38, our recommended reference is `human_GRCh38_no_alt_analysis_set`. This [README](https://github.com/PacificBiosciences/reference_genomes/blob/main/reference_genomes/human_GRCh38_no_alt_analysis_set/README.md) explains exactly what is included.
- **GRCh37/hg19**: Use configuration files from the `meta` directory with the `GRCh37` substring and set the genome flag to `"37"`
  - For GRCh37/hg19, our recommended reference is `human_hs37d5`. This [README](https://github.com/PacificBiosciences/reference_genomes/blob/main/reference_genomes/human_hs37d5/README.md) explains exactly what is included.

⚠️ **Warning**: Using a different GRCh37/GRCh38 reference set than the recommendations above (e.g., different contig naming, or references that include/exclude ALT/decoy contigs) can cause mismatches with PTCP `meta` files (BED/VCF/YAML), leading to runtime errors (e.g., missing contigs) and/or incorrect or incomplete results.
...
It can be invoked as a Docker container or a Singularity/Apptainer image file. Instructions for setting up both are provided in the [PTCP on HPC page](./PTCP_on_HPC.md#2-set-up-the-image-of-ptcp-dependencies).
...
| `docker_smrttools`         | `String`      | URI or tarball path for the Docker image                                                                      | 7     |
