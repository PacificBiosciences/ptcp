...
- **`bam_id`**: Unique sample ID, derived by stripping `.hifi_reads.`, `.fail_reads.`, or `.reads` from `bam_name` (e.g., `some_sample.bc42`). Must match the prefixes of all associated BAM files.
...
PTCP is a WDL workflow that relies on many software packages. These packages are containerized in an image for PTCP to use during runtime. A pre-built Docker image of these dependencies can be found at: quay.io/pacbio/ptcp.
...
```
{
  ...
  "ptcp.paraphase_annotation_vcf": "meta/variant_list/variant_list.GRCh38.vcf", <- Optional
