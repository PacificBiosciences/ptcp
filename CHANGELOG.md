...
- Documentation updates focused on running and interpreting PTCP:
  - Clarified how sample `sex` is used (including chrX behavior in TRGT/Paraphase/F8 outputs).
  - Added troubleshooting for unexpectedly low locus coverage caused by unsupported reference builds (ALT/decoy contigs).
  - Clarified that PTCP requires demultiplexed BAM inputs.
  - Documented recommended reference sets for supported genomes: `human_GRCh38_no_alt_analysis_set` (GRCh38) and `human_hs37d5` (GRCh37/hg19).
